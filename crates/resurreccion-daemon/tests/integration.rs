...
fn workspace_list_returns_empty_array_on_fresh_store() {
    ...
    let request = Envelope::ok("test-workspace-list", "workspace.list", serde_json::json!(null));

    let response = send_envelope(&socket, &request);
    assert!(response.ok, "workspace.list response ok field should be true");
    // Body should be an empty array with no workspaces
    assert!(response.body.is_array(), "workspace.list body should be an array, got: {:?}", response.body);
...
fn send_envelope(socket: &std::path::Path, env: &Envelope) -> Envelope {
    let mut stream = UnixStream::connect(socket)
        .expect(&format!("failed to connect to socket: {}", socket.display()));
